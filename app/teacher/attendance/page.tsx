...
import { teacherCanAccessClass, getTeacherClasses } from '@/lib/teacher-classes'
...
async function saveAttendance(formData: FormData) {
  ...
  const allowed = await teacherCanAccessClass(db, user.id, classId)
  if (!allowed) return
...
export default async function TeacherAttendance({
  ...
  searchParams: Promise<{ class_id?: string; date?: string; saved?: string }>
}) {
  ...
  return (
    <div className="h-full flex flex-col p-6 max-w-5xl mx-auto">
      ...
      {sp.saved && (
        <div className="mb-4 flex-shrink-0 p-3 bg-emerald-50 border border-emerald-200 rounded-lg text-sm text-emerald-700">
          ✅ Présences enregistrées avec succès.
        </div>
      )}

      {/* Filters */}
      <form method="GET" className="flex flex-wrap gap-3 mb-5 flex-shrink-0">
        <div>
      ...
      {/* Attendance form — fills all remaining vertical space */}
      {selectedClass && students.length > 0 && (
        <form action={saveAttendance} className="flex-1 flex flex-col min-h-0 gap-4">
          <input type="hidden" name="class_id" value={selectedClass} />
          <input type="hidden" name="date" value={selectedDate} />

          {/* Card grows to fill remaining space; student list scrolls inside */}
          <div className="bg-white rounded-xl shadow-sm flex-1 flex flex-col min-h-0 overflow-hidden">
            <div className="px-5 py-4 border-b bg-gray-50 flex items-center justify-between flex-shrink-0">
            ...
            {/* Only this div scrolls — single scrollbar */}
            <div className="divide-y divide-gray-50 overflow-y-auto flex-1">
              {students.map((student: any, idx: number) => {
                ...
                return (
                  <div
                    key={student.id}
                    className={`flex items-center gap-4 px-5 py-3 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}
                  >
                    <span className="w-6 text-gray-400 text-sm text-right flex-shrink-0">{idx + 1}</span>
                    ...
                    <div className="flex gap-2 flex-shrink-0">
                      {(['present', 'absent', 'late', 'excused'] as const).map((s) => {
                        ...
                        return (
                          <label
                            key={s}
                            className={`cursor-pointer border rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${
                              current === s ? colors[s] : inactiveColors[s]
                            }`}
                          >
                            <input
                              type="radio"
                    ...
                    <input
                      ...
                      defaultValue={getNotes(student.id)}
                      placeholder="Note…"
                      className="border border-gray-400 rounded-lg px-3 py-1.5 text-xs text-gray-600 w-28 flex-shrink-0 focus:ring-1 focus:ring-blue-500 focus:border-transparent"
                    />
                  </div>
                )
              })}
      ...
      {!selectedClass && (
        <div className="bg-white rounded-xl shadow-sm flex-1 flex flex-col items-center justify-center gap-3">
