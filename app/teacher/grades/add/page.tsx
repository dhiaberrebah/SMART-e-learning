...
async function addGrade(formData: FormData) {
  ...
  if (error) redirect(`/teacher/grades/add?error=${encodeURIComponent(error.message)}`)
  redirect('/teacher/grades?saved=1')
}

export default async function AddGradePage({ searchParams }: { searchParams: Promise<{ class_id?: string; error?: string }> }) {
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <form action={addGrade} className="space-y-5">
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Élève <span className="text-red-500">*</span></label>
            <select name="student_id" required
              className="w-full border border-gray-400 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent">
              <option value="">Sélectionner un élève</option>
              {(students as any[] ?? []).map((s: any) => <option key={s.id} value={s.id}>{s.full_name}</option>)}
            </select>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Matière <span className="text-red-500">*</span></label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Note obtenue <span className="text-red-500">*</span></label>
              <input type="number" name="grade_value" required min="0" step="0.5" placeholder="ex: 15"
                className="w-full border border-gray-400 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Note maximale</label>
              <input type="number" name="max_grade" min="1" step="0.5" defaultValue="20" placeholder="20"
                className="w-full border border-gray-400 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea name="notes" rows={3} placeholder="Remarques, commentaires…"
          ...
          <div className="flex gap-3 pt-2">
            ...
            <a href="/teacher/grades" className="px-5 py-2.5 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors text-sm">
              Annuler
            </a>
          </div>
        </form>
